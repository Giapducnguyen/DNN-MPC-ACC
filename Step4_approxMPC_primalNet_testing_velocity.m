...
%% Loading data
load("primalNet_training_velocity_August_24_Trial_01.mat",'net_primal_v')
...
actual_gapTest_primal_v = nan(num_testRuns,1);
relative_gapTest_primal_v = nan(num_testRuns,1);

% optimalValueTest_v = nan(num_testRuns,1);
x_nn_test_v = nan(nx+nd+nu+1,num_testRuns);
y_nn_test_v = nan(Np*nu,num_testRuns);

U_test_v = nan(Np*nu+n_epsilon,num_testRuns);
num_infeas_v = 0;
...
while ii <= num_testRuns
    ...
    if dk > d0+tau_h*vfk
        ...
        if exitflag_v <= 0
            num_infeas_v = num_infeas_v + 1;
        else
            ii
            % % control policy by NN
            uNN_v = net_primal_v(param0_v(1:6));

            % % project to guarantee feasibility
            %{
            polfeas_v = Polyhedron('A',Uad_v.A,'b',Uad_v.b-Uad_v.B*param0_v);
            polfeas_v.minHRep();
            try
            ...
            objective_onMPCTest_v = 0.5*dU_v'*H_v*dU_v + q_v'*dU_v;
            % % compute the cost using NN solution
            objective_approxMPCTest_v = 0.5*U_test_v(:,ii)'*H_v*U_test_v(:,ii) + q_v'*U_test_v(:,ii);
            % % Calculate the objective value gaps
            actual_gapTest_primal_v(ii,1) = norm(objective_onMPCTest_v-objective_approxMPCTest_v);
            relative_gapTest_primal_v(ii,1) = norm(objective_onMPCTest_v-objective_approxMPCTest_v)/norm(objective_onMPCTest_v);
            % % update counter
            ii = ii+1;
        end
    end
...
vA28_01 = sum(relative_gapTest_primal_v(:,1)<0.1/100)/num_testRuns*100;
vA28_05 = sum(relative_gapTest_primal_v(:,1)<0.5/100)/num_testRuns*100;
vA28_1 = sum(relative_gapTest_primal_v(:,1)<1/100)/num_testRuns*100;
vA28_5 = sum(relative_gapTest_primal_v(:,1)<5/100)/num_testRuns*100;
vA28_10 = sum(relative_gapTest_primal_v(:,1)<10/100)/num_testRuns*100;

disp(['online/offline gap rate below 0.1%: ' num2str(sum(relative_gapTest_primal_v(:,1)<0.1/100)/num_testRuns*100) '%'])
disp(['online/offline gap rate below 0.5%: ' num2str(sum(relative_gapTest_primal_v(:,1)<0.5/100)/num_testRuns*100) '%'])
disp(['online/offline gap rate below 1%: '   num2str(sum(relative_gapTest_primal_v(:,1)<1/100)/num_testRuns*100) '%'])
disp(['online/offline gap rate below 5%: '   num2str(sum(relative_gapTest_primal_v(:,1)<5/100)/num_testRuns*100) '%'])
disp(['online/offline gap rate below 10%: '   num2str(sum(relative_gapTest_primal_v(:,1)<10/100)/num_testRuns*100) '%'])
