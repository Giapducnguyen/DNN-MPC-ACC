...
Qe_v = diag([1,0.5]);    % 0.3 achieving set velocity 

Qe_s = diag([1,0.5]);    % 0.3 maintaining safe distance
...
max_d = Inf;            % maximum sensing range
...
n_epsilon = 1; % number of slack variables
...
% % Initializations with initial loggings
...
% Deep NN
xf_log_NN = zeros(nx,Nsim+1);
xf_log_NN(:,1) = [xf0; vf0; af0];
% Implicit
...
% % Intervehicle states
% Deep NN
xInter_log_NN = zeros(nx,Nsim+1);
xInter_log_NN(:,1) = [xp_log(1,1)-xf_log_NN(1,1); xf_log_NN(2,1); xf_log_NN(3,1)];
% Implicit
...
% % Execution time
% Deep NN
executionTime_NN = zeros(1,Nsim);
% Implicit
executionTime_impl = zeros(1,Nsim);

% % ACC vehicle control logging
% Deep NN
uf_log_NN = zeros(nu,Nsim);
duf_log_NN = zeros(nu,Nsim);
% Implicit
uf_log_impl = zeros(nu,Nsim);
duf_log_impl = zeros(nu,Nsim);

% % Objective value loggings
obj_impl_log = nan(1,Nsim);
obj_NN_log = nan(1,Nsim);

% % safe distance logging
% Deep NN
safeDistance_NN = zeros(1,Nsim+1);
safeDistance_NN(1) = d0 + tau_h*xf_log_NN(2,1);
% Implicit
...
% Deep NN
velocityControlTime_NN = zeros(1,Nsim);
% Implicit
...
% Deep NN
infeas_v_NN = zeros(Nsim,1);
infeas_s_NN = zeros(Nsim,1);
% Implicit
infeas_v_impl = zeros(Nsim,1);
infeas_s_impl = zeros(Nsim,1);

% % salck variable records
% Deep NN
slack_logging_v_NN = zeros(n_epsilon,Nsim);
slack_logging_s_NN = zeros(n_epsilon,Nsim);
% Implicit
slack_logging_v_impl = zeros(n_epsilon,Nsim);
slack_logging_s_impl = zeros(n_epsilon,Nsim);

% % main loop
for k = 1:Nsim
    % % Reference updates
    % safe distance - Deep NN
    d_ref_NN = d0 + tau_h*xf_log_NN(2,k);
    safeDistance_NN(k+1) = d0 + tau_h*xf_log_NN(2,k);
    ...
    if k == 1
        ...
        % u_prev_NN = uf_log_NN(:,k-1); % max(min_u,min(uf_log_NN(:,k),max_u))
        u_prev_NN = max(min_u,min(uf_log_NN(:,k-1),max_u));
...
%     Time_NN = tic;
    %
    if xInter_log_NN(1,k) <= safeDistance_NN(k+1)    % SPACE CONTROL
        velocityControlTime_NN(1,k) = 0;
        tic;
        dU_NN_s = pNet_s_CodeGen([xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;d_ref_NN]);
        executionTime_NN(k) = toc;
%         
%         polfeas_s = Polyhedron('A',Uad_s.A,'b',Uad_s.b-Uad_s.B*[xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;d_ref_NN;0]);
% %         polfeas_s.minHRep();
%         try
%             dU_NN_temp = polfeas_s.project(dU_NN_s).x;
%         catch
%             dU_NN_temp = dU_NN_s;
%         end
%         du_NN = dU_NN_temp(1,1);
        du_NN = dU_NN_s(1,1);

        % objective
        param0_s = [xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;d_ref_NN;0];
        q_s = B_bar_s'*Q_bar_s*A_bar_s*param0_s;
        % obj_NN = 0.5*dU_NN_temp'*H_s*dU_NN_temp + q_s'*dU_NN_temp;
        obj_NN = 0.5*dU_NN_s'*H_s*dU_NN_s + q_s'*dU_NN_s;
        
    else % velocity control
        velocityControlTime_NN(1,k) = 1;
        tic;
        dU_NN_v = pNet_v_CodeGen([xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;v_ref(k)]);
        executionTime_NN(k) = toc;
        %
%         polfeas_v = Polyhedron('A',Uad_v.A,'b',Uad_v.b-Uad_v.B*[xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;v_ref(k);0]);
% %         polfeas_v.minHRep();
%         try
%             dU_NN_temp = polfeas_v.project(dU_NN_v).x;
%         catch
%             dU_NN_temp = dU_NN_v; %dU_NN;
%         end
%         du_NN = dU_NN_temp(1,1);
        du_NN = dU_NN_v(1,1);

        % objective
        param0_v = [xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;d_ref_NN;0];
        q_v = B_bar_v'*Q_bar_v*A_bar_v*param0_v;
        % obj_NN = 0.5*dU_NN_temp'*H_v*dU_NN_temp + q_v'*dU_NN_temp;
        obj_NN = 0.5*dU_NN_v'*H_v*dU_NN_v + q_v'*dU_NN_v;
    end
    %}
    %{
    dU_NN_s = pNet_s_CodeGen([xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;d_ref_NN]);
    dU_NN_v = pNet_v_CodeGen([xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;v_ref(k)]);
    hv = 0.2; %0.25;
    heavisideStep = 1/(1+exp(-2*hv*(xInter_log_NN(1,k) - safeDistance_NN(1,k+1) + 0*xf_log_NN(2,k)))); % d0 + tau_h*xf_log_NN(2,k) % safeDistance_NN(1,k+1)
    du_NN = ((1-heavisideStep)*dU_NN_s(1,1) + (heavisideStep)*dU_NN_v(1,1));

    if xInter_log_NN(1,k) <= d0 + tau_h*xf_log_NN(2,k) %- 0.15*xf_log_NN(2,k)
        velocityControlTime_NN(1,k) = 0;
%         dU_NN_s = pNet_s_CodeGen([xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;d_ref_NN]);
% %         dU_NN_v = pNet_v_CodeGen([xInter_log_NN(:,k);xp_log(2,k);u_prev_NN;v_ref(k)]);
%         du_NN = dU_NN_s(1,1);
...
%         du_NN = dU_NN_v(1,1);
    ...
    %}
%     executionTime_NN(k) = toc(Time_NN);

    % duf_log_NN(:,k) = max(min_du, min(du_NN, max_du));
    % uf_log_NN(:,k) = max(min_u, min(u_prev_NN + du_NN, max_u));
    duf_log_NN(:,k) = du_NN;
    uf_log_NN(:,k) = u_prev_NN + du_NN;
...
% % % % -----------------------------------------------------------% % % % 
    ...
    % % Implicit
    %
    if xInter_log_impl(1,k) <= safeDistance_impl(k+1)   % space control
        velocityControlTime_impl(1,k) = 0;
        param0_s = [xInter_log_impl(:,k);xp_log(2,k);u_prev_impl;d_ref_impl;0];
        q_s = B_bar_s'*Q_bar_s*A_bar_s*param0_s;
        tic
%         [dU_s,exitflag_s,iA0_s,~] = mpcActiveSetSolver(H_s,q_s,Uad_s.A,Uad_s.b-Uad_s.B*param0_s,Uad_s.Ae,Uad_s.be,iA0_s,options);
        ...
        executionTime_impl(k) = toc;
        if exitflag_s <= 0
            du_impl = 0;
            obj_impl = nan;
        else
            du_impl = dU_s(1,1);
            slack_logging_s_impl(:,k) = dU_s(end,end);
            % objective
            obj_impl = 0.5*dU_s'*H_s*dU_s + q_s'*dU_s;
        end
...
%         [dU_v,exitflag_v,iA0_v,~] = mpcActiveSetSolver(H_v,q_v,Uad_v.A,Uad_v.b-Uad_v.B*param0_v,Uad_v.Ae,Uad_v.be,iA0_v,options);
        ...
        if exitflag_v <= 0
            ...
            du_impl = dU_v(1,1);
            slack_logging_v_impl(:,k) = dU_v(end,end);
            % objective
            obj_impl = 0.5*dU_v'*H_v*dU_v + q_v'*dU_v;
        end
    end
    %
    %{
    ...
    if status_val_s ~= 1
        ...
        du_impl_s = res_prim_s(1,1);
    end
    [res_prim_v,res_dual_v,status_val_v,iter_v,run_time_v] = emosqp_v('solve');
    if status_val_v ~= 1
        du_impl_v = 0;
    else
        du_impl_v = res_prim_v(1,1);
    end

    hv = 1;
    heavisideStep = 1/(1+exp(-2*hv*(xInter_log_impl(1,k) - safeDistance_impl(1,k+1) + 0*xf_log_NN(2,k))));
    du_impl = (1-heavisideStep)*du_impl_s(1,1) + (heavisideStep)*du_impl_v(1,1);
    %}
    ...
    % % following (ACC) vehicle
    % Deep NN
    xf_log_NN(:,k+1) = indiVehDyn(Ts,tau_f,xf_log_NN(:,k),max(min_u,min(uf_log_NN(:,k),max_u)));
    % Implicit
    xf_log_impl(:,k+1) = indiVehDyn(Ts,tau_f,xf_log_impl(:,k),uf_log_impl(:,k));
    % % Intervehicle states
    % Deep NN
    xInter_log_NN(:,k+1) = [xp_log(1,k+1)-xf_log_NN(1,k+1); xf_log_NN(2,k+1); xf_log_NN(3,k+1)];
    % Implicit
...
nexttile
hold on
plot(Time_log,xInter_log_NN(1,:),'LineWidth',1,'LineStyle','-','Color','m');
plot(Time_log,safeDistance_NN,'LineWidth',1,'LineStyle','--','Color','m');
plot(Time_log,xInter_log_impl(1,:),'LineWidth',1,'LineStyle','-','Color','b');
plot(Time_log,safeDistance_impl,'LineWidth',1,'LineStyle','--','Color','b');
ylabel('$d (m)$','Interpreter','latex');
title('Distance between two cars','Interpreter','latex');
...
plot(Time_log,xf_log_NN(2,:),'LineWidth',1,'LineStyle','-','Color','m');
...
plot(Time_log,xf_log_NN(3,:),'LineWidth',1,'LineStyle','-','Color','m');
...
nexttile
hold on
% stairs(Time_log(1:end-1),max(min_u,min(uf_log_NN,max_u)),'LineWidth',1,'LineStyle','-','Color','m');
stairs(Time_log(1:end-1),uf_log_NN,'LineWidth',1,'LineStyle','-','Color','m');
...
nexttile
hold on
% stairs(Time_log(1:end-1),max(min_du,min(duf_log_NN,max_du)),'LineWidth',1,'LineStyle','-','Color','m');
stairs(Time_log(1:end-1),duf_log_NN,'LineWidth',1,'LineStyle','-','Color','m');
...
nexttile
hold on
stairs(Time_log(1:end-1),executionTime_NN,'LineWidth',1,'LineStyle','-','Color','m');
...
% stairs(Time_log(1:end-1),executionTime_impl_qpoases,'LineWidth',1,'LineStyle','--','Color','r');
ylabel('$t_{exe} (sec)$','Interpreter','latex');
title('Execution time (zoomed on y axis)','Interpreter','latex');
% legend({'ACC-DeepNN','ACC-implicit'},'Interpreter','latex','Location','best');
grid on; box on;
ylim([0 0.018])
ax = gca; ax.FontSize = 14;
...
nexttile
hold on
plot(Time_log(1:end-1),velocityControlTime_NN,'LineWidth',1,'LineStyle','-','Color','m')
plot(Time_log(1:end-1),velocityControlTime_impl,'LineWidth',1,'LineStyle','--','Color','b')
ylabel('State','Interpreter','latex');
xlabel('Time (seconds)','Interpreter','latex');
title('Velocity Control Mode Activation Time','Interpreter','latex');
legend({'ACC-DeepNN','ACC-implicit'},'Interpreter','latex','Location','best');
grid on; box on;
ax = gca; ax.FontSize = 14;
