...
%     up_curr = 0.3*(1+k/300)*sin(0.2*(k-1)*Ts);
    ...
    if k == 1
        up_curr = 0;
    elseif k <= (10/Ts) % 10 [sec]
        up_curr = -1; %-0.1*(k*Ts);
    elseif k <= (20/Ts) % 20 [sec]
        up_curr = -1.5;
    elseif k <= (40/Ts) % 40 [sec]
        up_curr = 0; %(-0.225)*(k*Ts) + 6;
    elseif k <= (60/Ts) % 60 [sec]
        up_curr = 2; %-3;
    elseif k <= (75/Ts) % 75 [sec]
        up_curr = 0; %3/8.8*(k*Ts) - 225/11;
    elseif k <= (92.5/Ts) %92.5 [sec]
...
%         [dU,exitflag,iA0_s,~] = mpcActiveSetSolver(H_s,q_s,Uad_s.A,Uad_s.b-Uad_s.B*param0_s,Uad_s.Ae,Uad_s.be,iA0_s,options);
        ...
        if exitflag_s <= 0
            ...
            du_impl = dU_s(1,1);
            slack_logging_s(:,k) = dU_s(end-n_epsilon+1:end,1);
        end
...
%         [dU,exitflag,iA0_v,~] = mpcActiveSetSolver(H_v,q_v,Uad_v.A,Uad_v.b-Uad_v.B*param0_v,Uad_v.Ae,Uad_v.be,iA0_v,options);
        ...
        if exitflag_v <= 0
            ...
            du_impl = dU_v(1,1);
            slack_logging_v(:,k) = dU_v(end-n_epsilon+1:end,1);
        end
    end
    %}
